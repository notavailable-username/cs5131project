...
class MatchingNetworkInference:
    """
    Class for performing inference with a trained Matching Network model.
    Provides core functions for setting up support examples and making predictions.
    """
    def __init__(self, model_path, device=None):
        ...
        Args:
            ...
            device: Device to run inference on (default: auto-select)
        """
        # Set device
        if device is None:
        ...
        # Initialize support data attributes
        self.support_images = None
        self.support_labels_one_hot = None
    ...
    def _load_model(self, model_path):
        ...
        n_way = hyperparams.get('n_way', 5)
        n_support = hyperparams.get('n_support', 5)
        n_query = hyperparams.get('n_query', 15)
        feat_dim = hyperparams.get('feat_dim', 64)
        lstm_layers = hyperparams.get('lstm_layers', 1)
        unrolling_steps = hyperparams.get('unrolling_steps', 2)
        use_fce = hyperparams.get('use_fce', True)
    ...
    def set_support(self, support_image_paths, support_labels=None):
        """
        Set up a new support set with the provided images and labels.
        Replaces any existing support set.
        
        Args:
            support_image_paths: Path or list of paths to support images
            support_labels: Label or list of labels for support images (optional)
                If None, each support image is treated as its own class
        """
        # Convert single path/label to list if needed
        if isinstance(support_image_paths, str):
            support_image_paths = [support_image_paths]
        
        if support_labels is None:
            # Each support image is its own class
            support_labels = list(range(len(support_image_paths)))
        elif not isinstance(support_labels, (list, tuple)):
            # Single label provided, convert to list
            support_labels = [support_labels] * len(support_image_paths)
        
        # Ensure lengths match
        if len(support_image_paths) != len(support_labels):
            raise ValueError("Number of support images and labels must match")
        
        # Get unique class labels and create mapping
        self.unique_labels = sorted(set(support_labels))
        n_way = len(self.unique_labels)
        self.label_to_idx = {label: i for i, label in enumerate(self.unique_labels)}
        
        # Load support images
        support_images = []
        support_labels_one_hot = torch.zeros(len(support_image_paths), n_way)
        
        for i, (path, label) in enumerate(zip(support_image_paths, support_labels)):
            img = self._load_and_process_image(path)
            support_images.append(img)
            support_labels_one_hot[i, self.label_to_idx[label]] = 1
        
        # Store processed support data
        self.support_images = torch.stack(support_images).to(self.device)
        self.support_labels_one_hot = support_labels_one_hot.to(self.device)
        
        logger.info(f"Support set configured with {len(support_image_paths)} images and {n_way} classes")
    
    def add_support(self, support_image_paths, support_labels=None):
        """
        Add more support examples to the existing support set.
        If no support set exists, creates a new one.
        
        Args:
            support_image_paths: Path or list of paths to support images to add
            support_labels: Label or list of labels for the support images (optional)
                If None, labels will be assigned starting from the current max label + 1
        """
        # Convert single path/label to list if needed
        if isinstance(support_image_paths, str):
            support_image_paths = [support_image_paths]
        ...
        if self.support_images is None:
            # No existing support set, create a new one
            self.set_support(support_image_paths, support_labels)
            return
        
        # Handle support labels
        if support_labels is None:
            # Assign new classes starting from max label + 1
            start_label = max(self.unique_labels) + 1 if self.unique_labels else 0
            support_labels = [start_label + i for i in range(len(support_image_paths))]
        elif not isinstance(support_labels, (list, tuple)):
            # Single label provided, convert to list
            support_labels = [support_labels] * len(support_image_paths)
        ...
        # Get updated unique class labels and create new mapping
        new_unique_labels = sorted(set(self.unique_labels + list(set(support_labels))))
        n_way = len(new_unique_labels)
        new_label_to_idx = {label: i for i, label in enumerate(new_unique_labels)}
        
        # Convert existing support labels to new one-hot format
        new_support_labels_one_hot = torch.zeros(
            self.support_labels_one_hot.size(0), n_way, device=self.device
        )
        
        # Map old labels to new indices
        for old_idx, old_label in enumerate(self.unique_labels):
            new_idx = new_label_to_idx[old_label]
            # Copy the one-hot vectors to their new positions
            mask = self.support_labels_one_hot[:, old_idx] == 1
            new_support_labels_one_hot[mask, new_idx] = 1
        
        # Process new support images
        new_images = []
        new_labels_one_hot = torch.zeros(len(support_image_paths), n_way, device=self.device)
        
        for i, (path, label) in enumerate(zip(support_image_paths, support_labels)):
            img = self._load_and_process_image(path)
            new_images.append(img)
            new_labels_one_hot[i, new_label_to_idx[label]] = 1
        
        # Combine existing and new support data
        self.support_images = torch.cat([
            self.support_images, 
            torch.stack(new_images).to(self.device)
        ], dim=0)
        
        self.support_labels_one_hot = torch.cat([
            new_support_labels_one_hot,
            new_labels_one_hot
        ], dim=0)
        
        # Update class mappings
        self.unique_labels = new_unique_labels
        self.label_to_idx = new_label_to_idx
    ...
    def predict(self, query_image_paths, k=None, q=None):
        ...
        """
        # Check if support set is configured
        if self.support_images is None or self.support_labels_one_hot is None:
            raise ValueError("Support set not configured. Call set_support() first.")
        ...
        for i in range(len(query_image_paths)):
            ...
            predicted_label = self.unique_labels[predicted_idx]
            confidence = confidences[i].item()
            results.append((predicted_label, confidence))
        
        # Return a single result if input was a single path
        if single_input:
            return results[0]
        return results
